...
def send_post_back_office(proc_name, package_name, request_body, request_mapping_function, response_mapping_function, jwt_token, return_code='PV_RETURN_CODE_O'):
    ...
    if response is None or (response[return_code] and response[return_code] is not 0):
        logger.error(f"Fsbid call to '{url}' failed.")
        return None
