...
def user_bids(employee_id, jwt, position_id=None):
    ...
    bids = requests.get(url, headers={'Authorization': jwt, 'Content-Type': 'application/json'}, verify=False).json()
...
def bid_on_position(userId, jwt, employeeId, cyclePositionId):
    ...
    response = requests.post(url, data={"perdet_seq_num": employeeId, "cp_id": cyclePositionId, "userId": userId}, headers={'Authorization': jwt, 'Content-Type': 'application/json'}, verify=False)
...
def remove_bid(employeeId, cyclePositionId, jwt):
    ...
    return requests.delete(url, headers={'Authorization': jwt, 'Content-Type': 'application/json'}, verify=False)
...
def get_projected_vacancies(query, jwt, host=None):
    ...
    response = requests.get(url, headers={'Authorization': jwt, 'Content-Type': 'application/json'}, verify=False).json()
...
def get_bid_seasons(bsn_future_vacancy_ind, jwt):
    ...
    bid_seasons = requests.get(url, headers={'Authorization': jwt, 'Content-Type': 'application/json'}, verify=False).json()
